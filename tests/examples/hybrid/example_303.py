...
def run_example(delay_short: int = DELAY_SHORT):
    ...
    print_delayed(
        BR
        + "Alice receives the ASYMMETRICALY ENCRYPTED SYMMETRIC_KEY"
        + " - ( HANDSHAKE )"
    )
    ...
    print_delayed(BR + "Bob receives the ENCRYPTED MESSAGE and SIGNATURE..." + BR)
    decrypted_message = bob.key_symmetric.decrypt(encrypted_message, bob.key_exchange)
    print_delayed(
    ...
    print()
    print_delayed(
        f"  Bob checks SIGNATURE of the MESSAGE with PUBLIC_KEY(Alice):"
        + BR
        + f"       = signature("
        + alice.signature[:10]
        + ") -> OK"
